...
public class Storage {
	...
	public static Feedback deleteTask(int itemId){
		...
		if (numberOfTask < itemId){
			feedbackObject.setMessageShowToUser(String.format(ERROR_INDEX_OUT_OF_RANGE, itemId));
			return feedbackObject;
		}
	...
	public static Feedback displayTask(){
		...
		if (listOfTask.size() == 0){
			feedbackObject.setMessageShowToUser(String.format(MESSAGE_EMPTY_TASK));
		}else {
			for (int index = 0; index < listOfTask.size(); index++){
				...
				if(date == null){
					feedbackObject.setMessageShowToUser(String.format(MESSAGE_DISPLAY_WITHOUT_DATE, index + 1, description));
				}
				else{
					feedbackObject.setMessageShowToUser(String.format(MESSAGE_DISPLAY_WITH_DATE, index + 1, description, date));
				}
			}
		}
		return feedbackObject;
	...
	/*public static Feedback checkStatus(){
		...
		for (int itemId = 0; itemId < listOfTask.size(); itemId++){
			if (listOfTask.get(itemId).getTime().before(now)){
				...
				history.add(removedTask);
				feedbackObject.setMessageShowToUser(String.format(MESSAGE_EXPIRED, removedTask.toString()));
			}
		}
		return feedbackObject;
	}*/
