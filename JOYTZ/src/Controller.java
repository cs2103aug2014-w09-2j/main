...
public class Controller {
	...
	private static ExecutableCommand analyzeInput(Command inputCommandObject)
		...
		ExecutableCommand parsedCommand = Analyzer
				.runAnalyzer(inputCommandObject);

		return parsedCommand;
	}

	private static String proceedFeedback(Feedback feedback,
			ExecutableCommand commandObj) {
		/*
		 * String action = ""; String taskName = ""; Date time; String location;
		 */
		String outputString;

		/*
		 * if (feedback.getResult()) { action = feedback.getMessageShowToUser();
		 * if (commandObj.getDescription() != null) { taskName =
		 * commandObj.getDescription(); } else { taskName = ""; } time =
		 * command.getTime(); location = command.getLocation(); }
		 */
